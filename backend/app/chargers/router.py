...
async def list_chargers(
    ...
    return await service.list_chargers(
        pool, user.org_id,
        status_filter=status, health_below=health_below, page=page, limit=limit,
    )


@router.post("", response_model=schemas.ChargerOut, status_code=201)
async def create_charger(
    req: schemas.ChargerCreate, pool: DBPool, user: CurrentUser,
):
    return await service.create_charger(pool, user.org_id, req)


@router.get("/{cp_id}", response_model=schemas.ChargerDetailResponse)
async def get_charger(cp_id: str, pool: DBPool, user: CurrentUser):
    return await service.get_charger(pool, user.org_id, cp_id)


@router.get("/{cp_id}/health", response_model=schemas.HealthResponse)
async def get_health(
    cp_id: str, pool: DBPool, user: CurrentUser,
...
async def get_telemetry(
    ...
    return await service.get_telemetry(pool, user.org_id, cp_id, frm, to, event_type, limit)
...
async def bulk_import(
    req: schemas.BulkImportRequest, pool: DBPool, user: CurrentUser,
):
...
async def send_command(
    cp_id: str, req: schemas.CommandRequest,
    pool: DBPool, user: CurrentUser,
):
